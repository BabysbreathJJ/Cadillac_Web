...
angular.module('myApp.homepage')
    .controller('DeletedCtrl', DeletedCtrl)
    .factory('CarDeletedService', CarDeletedService)
...
function CarDeletedService($http, BaseUrl) {
...
/** @ngInject */
function DeletedCtrl($scope, $filter, editableOptions, editableThemes, CarDeletedService) {
    $scope.smartTablePageSize = 10;
    $scope.pagination = {currentPage: 1};

    $scope.getCars = function (pageNo) {
        CarDeletedService.getCars(pageNo).success(function (data, status) {
    ...
    CarDeletedService.getLines().success(function (data, status) {
        $scope.lines = data.data;
    });

    $scope.getConfigs = function (lineNo) {
        CarDeletedService.getConfig(lineNo).success(function (data, status) {
